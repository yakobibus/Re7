...
namespace re_7_joueur
{
	...
	public :
		...
		{
			for (int i = 0; i < 7; ++i) { _suiteDeLances[i] = -1; }
		}
		~Joueur() = default;
		...
		void affiche(void) { std::cout << "Pseudo : " << _pseudo.c_str() << std::endl; }
	private :
		std::string _pseudo;
		bool _estHorsJeux;
...
namespace re_7_partie
{
	...
	public :
		...
		{
			_Joueurs = new re_7_joueur::Joueur[_nbJoueurs]; for (int i = 0; i < _nbJoueurs; ++i) { _Joueurs[i].setPseudo(listePseudos[i]); }
		}
		...
		inline void affiche(void) { for (int i = 0; i < _nbJoueurs; ++i) { _Joueurs->affiche(); } }
	private :
